...
mantis::Encoder::Encoder(ros::NodeHandle& nh, uint8_t pin1, uint8_t pin2, int encoder_resolution)
  : nh_(nh)
  , encoder(pin1, pin2)
...
mantis::JointState mantis::Encoder::jointState()
{
...
double mantis::Encoder::angularPosition()
{
    return joint_state_.angular_position_;
}


double mantis::Encoder::angularVelocity()
{
    return joint_state_.angular_velocity_;
}

double mantis::Encoder::ticksToAngle(const int &ticks) const
{
