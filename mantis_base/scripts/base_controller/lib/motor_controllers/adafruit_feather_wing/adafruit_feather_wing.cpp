...
mantis::AdafruitMotorController::AdafruitMotorController(uint8_t motor_num, uint8_t addr)
{
...
void mantis::AdafruitMotorController::begin(uint16_t freq)
{
    motor_driver_.begin(freq);
}

void mantis::AdafruitMotorController::setSpeed(int value)
{
    if (value > 0)
    {
        pMotor_->run(FORWARD);
