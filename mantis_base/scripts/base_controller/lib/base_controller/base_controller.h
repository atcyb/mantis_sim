...
#ifndef mantis_BASE_CONTROLLER_H
#define mantis_BASE_CONTROLLER_H


#include <ros.h>
#include <mantis_msgs/EncodersStamped.h>
#include <mantis_msgs/WheelsCmdStamped.h>
#include <mantis_msgs/AngularVelocities.h>
#include <mantis_msgs/PIDStamped.h>
#include <std_msgs/Empty.h>
#include <sensor_msgs/JointState.h>

#include "mantis_base_config.h"
#include "encoder_mantis.h"
...
namespace mantis {
     ...
     * 
     * The BaseController communicates with the high level interface mantis_base::hardware_interface::RobotHW
     ...
     * (\ref mantis_msgs::WheelsCmdStamped on the "wheel_cmd_velocities" topic) with the 
     ...
     * \ref mantis::MotorControllerIntf. Each time a new \ref mantis_msgs::WheelsCmdStamped is received on the
     ...
     * two \ref mantis::Encoder objects (\ref encoder_left_ \ref encoder_right_)
     * are used to \ref read() the mantis::JointStates, stored in \ref joint_state_left_ and \ref joint_state_left_.
     * After reading the latest states (position and velocity), the values are published with \ref pub_measured_joint_states_ on 
     * "measured_joint_states" topic of type \ref sensor_msgs::JointState. The \ref mantis_base::hardware_interface::RobotHW
     ...
     * - /mantis/encoder_resolution
     * - /mantis/mobile_base_controller/wheel_radius
     * - /mantis/mobile_base_controller/linear/x/max_velocity
     * - /mantis/debug/base_controller
     * 
    ...
    public:
         ...
         * The only requirement is to implement the \ref mantis::MotorControllerIntf,
         ...
         *   - /mantis/mobile_base_controller/linear/x/max_velocity
         *   - /mantis/debug/base_controller
         * - Initialize mantis Wheel Encoders
         ...
         * \ref LastUpdateTime::command_received period on the /mantis/wheel_cmd_velocities topic.
         * 
         */
        void eStop();
         ...
         * (e.g. mantis::PID::error_ = \ref wheel_cmd_velocity_left_ - \ref measured_angular_velocity_left_)
         * 
         * The calculated PID ouput values are capped at -/+ MAX_RPM to prevent the PID from having too much error
         * The computed and capped PID values are then set using the \ref mantis::MotorControllerIntf::setSpeed method
        ...
        /**
         * @brief Callback method when a new \ref mantis_msgs::WheelsCmdStamped is received on the wheel_cmd_velocities topic.
         * 
         ...
         * 
         * This callback method receives \ref mantis_msgs::WheelsCmdStamped message object (\p cmd_msg) consisting of
         * \ref mantis_msgs::AngularVelocities, where commanded wheel joints for the left and right wheel are stored.
         ...
         * This is used for the eStop functionality. In case no mantis_msgs::WheelsCmdStamped messages are 
         ...
         */
        void commandCallback(const mantis_msgs::WheelsCmdStamped& cmd_msg);
         ...
         * Every time the mantis_bringup/launch/bringup.launch is launched, the high level
         ...
         */
        void pidLeftCallback(const mantis_msgs::PIDStamped& pid_msg);
         ...
         * @param pid_msg message containing the new PID constants
         */
        void pidRightCallback(const mantis_msgs::PIDStamped& pid_msg);
    ...
    private:
        ...
        mantis_msgs::EncodersStamped encoder_msg_;
        ...
        ros::Subscriber<mantis_msgs::WheelsCmdStamped, BaseController<TMotorController, TMotorDriver>> sub_wheel_cmd_velocities_;
        ...
        ros::Subscriber<mantis_msgs::PIDStamped, BaseController<TMotorController, TMotorDriver>> sub_pid_left_;
        ros::Subscriber<mantis_msgs::PIDStamped, BaseController<TMotorController, TMotorDriver>> sub_pid_right_;
        PID motor_pid_left_;
...
template <typename TMotorController, typename TMotorDriver>
using BC = mantis::BaseController<TMotorController, TMotorDriver>;


template <typename TMotorController, typename TMotorDriver>
mantis::BaseController<TMotorController, TMotorDriver>
    ::BaseController(ros::NodeHandle &nh, TMotorController* motor_controller_left, TMotorController* motor_controller_right)
    : nh_(nh)
...
template <typename TMotorController, typename TMotorDriver>
void mantis::BaseController<TMotorController, TMotorDriver>::setup()
{
    nh_.initNode();
...
template <typename TMotorController, typename TMotorDriver>
void mantis::BaseController<TMotorController, TMotorDriver>::init()
{
    nh_.loginfo("Get Parameters from Parameter Server");
    nh_.getParam("/mantis/encoder_resolution", &this->encoder_resolution_);
    String log_msg = String("/mantis/encoder_resolution: ") + String(encoder_resolution_);
    nh_.loginfo(log_msg.c_str());
    nh_.getParam("/mantis/mobile_base_controller/wheel_radius", &wheel_radius_);
    log_msg = String("/mantis/mobile_base_controller/wheel_radius: ") + String(wheel_radius_);
    nh_.loginfo(log_msg.c_str());
    nh_.getParam("/mantis/mobile_base_controller/linear/x/max_velocity", &max_linear_velocity_);
    log_msg = String("/mantis/mobile_base_controller/linear/x/max_velocity: ") + String(max_linear_velocity_);
    nh_.loginfo(log_msg.c_str());
    nh_.getParam("/mantis/debug/base_controller", &debug_);
    log_msg = String("/mantis/debug/base_controller: ") + String(debug_);
    nh_.loginfo(log_msg.c_str());

    nh_.loginfo("Initialize mantis Wheel Encoders");
...
template <typename TMotorController, typename TMotorDriver>
void mantis::BaseController<TMotorController, TMotorDriver>::commandCallback(const mantis_msgs::WheelsCmdStamped& cmd_msg)
{
    ...
    // This callback function receives mantis_msgs::WheelsCmdStamped message object
    // where mantis_msgs::AngularVelocities for both joints are stored
    wheel_cmd_velocity_left_ = cmd_msg.wheels_cmd.angular_velocities.joint[0];
    wheel_cmd_velocity_right_ = cmd_msg.wheels_cmd.angular_velocities.joint[1];

    // Used for the eStop. In case no mantis_msgs::WheelsCmdStamped messages are received on the wheel_cmd_velocities 
...
template <typename TMotorController, typename TMotorDriver>
void mantis::BaseController<TMotorController, TMotorDriver>::resetEncodersCallback(const std_msgs::Empty& reset_msg)
{
...
template <typename TMotorController, typename TMotorDriver>
void mantis::BaseController<TMotorController, TMotorDriver>::pidLeftCallback(const mantis_msgs::PIDStamped& pid_msg)
{
    // Callback function to update the pid values of the left motor
    // This callback function receives mantis_msgs::PID message object
    // where mantis_msgs::PID kp, ki, kd for one pid controller is stored
    motor_pid_left_.updateConstants(pid_msg.pid.kp, pid_msg.pid.ki, pid_msg.pid.kd);
    motor_pid_left_.updateConstants(pid_msg.pid.kp, pid_msg.pid.ki, pid_msg.pid.kd);
}

template <typename TMotorController, typename TMotorDriver>
void mantis::BaseController<TMotorController, TMotorDriver>::pidRightCallback(const mantis_msgs::PIDStamped& pid_msg)
{
    // Callback function to update the pid values of the left motor
    // This callback function receives mantis_msgs::PID message object
    // where mantis_msgs::PID kp, ki, kd for one pid controller is stored
...
template <typename TMotorController, typename TMotorDriver>
void mantis::BaseController<TMotorController, TMotorDriver>::read()
{
...
template <typename TMotorController, typename TMotorDriver>
void mantis::BaseController<TMotorController, TMotorDriver>::write()
{
    // https://www.arduino.cc/reference/en/language/functions/math/map/
    // map(value, fromLow, fromHigh, toLow, toHigh)
    // Map angular wheel joint velocity to motor cmd
    // The /mantis/mobile_base_controller/linear/x/max_velocity from the ROS parameter server 
...
template <typename TMotorController, typename TMotorDriver>
void mantis::BaseController<TMotorController, TMotorDriver>::eStop()
{
...
template <typename TMotorController, typename TMotorDriver>
void mantis::BaseController<TMotorController, TMotorDriver>::printDebug()
{
    /*
    String log_msg =
            String("\nRead:\n") +
...
#endif // mantis_BASE_CONTROLLER_H