#include <mantis_base/mantis_hw_interface.h>
...
namespace mantis_base
{
    mantisHWInterface::mantisHWInterface(ros::NodeHandle &nh, urdf::Model *urdf_model)
    ...
    { 
        ...
        // Get additional parameters from the mantis_base/config/base.yaml which is stored on the parameter server
        ...
        pub_wheel_cmd_velocities_ = nh_.advertise<mantis_msgs::WheelsCmdStamped>("wheel_cmd_velocities", 10);
        ...
        sub_encoder_ticks_ = nh_.subscribe("encoder_ticks", 10, &mantisHWInterface::encoderTicksCallback, this);
        sub_measured_joint_states_ = nh_.subscribe("measured_joint_states", 10, &mantisHWInterface::measuredJointStatesCallback, this);
    ...
    bool mantisHWInterface::init(ros::NodeHandle &root_nh, ros::NodeHandle &robot_hw_nh)
    {
        ROS_INFO("Initializing mantis Hardware Interface ...");
        ...
        ROS_INFO("... Done Initializing mantis Hardware Interface");

        return true;
    }

    void mantisHWInterface::read(const ros::Time& time, const ros::Duration& period)
    {
    ...
    void mantisHWInterface::write(const ros::Time& time, const ros::Duration& period)
    {
        ros::Duration elapsed_time = period;
        // Write to robot hw
        ...
        mantis_msgs::WheelsCmdStamped wheel_cmd_msg;
    ...
    bool mantisHWInterface::isReceivingMeasuredJointStates(const ros::Duration &timeout)
    {
    ...
    void mantisHWInterface::loadURDF(const ros::NodeHandle &nh, std::string param_name)
    {
    ...
    void mantisHWInterface::printState()
    {
    ...
    std::string mantisHWInterface::printStateHelper()
    {
        std::stringstream ss;
    ...
    std::string mantisHWInterface::printCommandHelper()
    {
        std::stringstream ss;
        std::cout.precision(15);
    ...
    void mantisHWInterface::encoderTicksCallback(const mantis_msgs::EncodersStamped::ConstPtr& msg_encoder)
    {
    ...
    void mantisHWInterface::measuredJointStatesCallback(const sensor_msgs::JointState::ConstPtr& msg_joint_states)
    {
        /// Update current encoder ticks in encoders array
        for (std::size_t i = 0; i < num_joints_; ++i)
        {
    ...
    double mantisHWInterface::ticksToAngle(const int &ticks) const
    {
    ...
    double mantisHWInterface::normalizeAngle(double &angle) const
    {
    ...
    double mantisHWInterface::linearToAngular(const double &distance) const
    {
        return distance / wheel_diameter_ * 2.0;
    }

    double mantisHWInterface::angularToLinear(const double &angle) const
    {
